...
@section('title', __('trans.edit_cleaner') . ' - ' . $cleaner->name)
...
@section('content')
    <div class="form-container">
        <div class="glass-card form-card">
            ...
            <form method="POST" action="{{ route('dashboard.cleaners.update', $cleaner) }}" class="user-form" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-header">
                    <h3>{{ __('trans.cleaner_information') }}</h3>
                    <p>{{ __('trans.update_cleaner_information_below') }}</p>
                </div>
                <div class="form-grid">
                    <div class="form-group">
                        <label for="name" class="form-label">{{ __('trans.full_name') }}<span
                                class="required">*</span></label>
                        <div class="input-wrapper">
                            <input type="text" id="name" name="name" value="{{ old('name', $cleaner->name) }}"
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            <input type="text" id="national_id" name="national_id"
                                value="{{ old('national_id', $cleaner->national_id) }}"
                                class="form-input @error('national_id') error @enderror"
                                placeholder="{{ __('trans.enter_national_id') }}">
                        </div>
                        @error('national_id')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            <input type="text" id="address" name="address"
                                value="{{ old('address', $cleaner->address) }}"
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            <input type="date" id="hire_date" name="hire_date"
                                value="{{ old('hire_date', $cleaner->hire_date ? \Illuminate\Support\Carbon::parse($cleaner->hire_date)->format('Y-m-d') : '') }}"
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            <select id="status" name="status" class="form-input @error('status') error @enderror"
                                required>
                                <option value="active" {{ old('status', $cleaner->status) == 'active' ? 'selected' : '' }}>
                                    {{ __('trans.active') }}</option>
                                <option value="inactive"
                                    {{ old('status', $cleaner->status) == 'inactive' ? 'selected' : '' }}>
                                    {{ __('trans.inactive') }}</option>
                            </select>
                        </div>
                        @error('status')
                            <span class="error-message">{{ $message }}</span>
                    ...
                    <div class="form-group">
                        ...
                        @if($cleaner->image)
                            <div style="margin-top: 1rem;">
                                <img src="{{ asset('storage/'.$cleaner->image) }}" alt="Cleaner Image" style="max-width: 120px; border-radius: 8px;">
                            </div>
                        @endif
                        @error('image')
                            <span class="error-message">{{ $message }}</span>
